...
public final class DFSAMinimizer {
  ...
  static <T, S> void unweightedMinimizeOld(DFSA<T, S> dfsa) {
    ...
    for (Map.Entry<UnorderedPair<DFSAState<T, S>, DFSAState<T, S>>, Boolean> unorderedPairBooleanEntry : stateUPairToDistinguished.entrySet()) {

      DFSAState<T, S> state1 = unorderedPairBooleanEntry.getKey().first;
      DFSAState<T, S> state2 = unorderedPairBooleanEntry.getKey().second;
      if (unorderedPairBooleanEntry.getValue().equals(Boolean.TRUE)) {
        continue;
      }
      // check if some input distinguishes this pair
      ...
      if (distinguishable) {
        ...
        markStack.add(unorderedPairBooleanEntry.getKey());
        while (!markStack.isEmpty()) {
        ...
        for (UnorderedPair<DFSAState<T, S>, DFSAState<T, S>> pendingUPair : pendingUPairs) {
          ...
          dependentList.add(unorderedPairBooleanEntry.getKey());
        }
      }
    }
    if (debug) {
    ...
    DisjointSet<DFSAState<T, S>> stateClasses = new FastDisjointSet<>(states);
    for (Map.Entry<UnorderedPair<DFSAState<T, S>, DFSAState<T, S>>, Boolean> unorderedPairBooleanEntry : stateUPairToDistinguished.entrySet()) {
      if (unorderedPairBooleanEntry.getValue().equals(Boolean.FALSE)) {
        DFSAState<T, S> state1 = unorderedPairBooleanEntry.getKey().first;
        DFSAState<T, S> state2 = unorderedPairBooleanEntry.getKey().second;
        stateClasses.union(state1, state2);
      }
