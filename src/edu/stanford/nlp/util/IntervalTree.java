...
public class IntervalTree<E extends Comparable<E>, T extends HasInterval<E>> extends AbstractCollection<T> {
    ...
    public static class TreeNode<E extends Comparable<E>, T extends HasInterval<E>> {
        ...
        public TreeNode(TreeNode<E,T> parent) {
            super();
            this.parent = parent;
        }
...
//      parent = null;
    ...
    public boolean add(final TreeNode<E, T> node, T target, double alpha) {
        if (target == null) return false;
        TreeNode<E, T> n = node;
        int depth = 0;
        int thresholdDepth = (node.size > 10) ? ((int) (-Math.log(node.size) / Math.log(alpha) + 1)) : 10;

        final E targetEnd = target.getInterval().getEnd();
    ...
    public static boolean isAlphaBalanced(TreeNode node, double alpha) {
        final TreeNode L = node.left;
        int leftSize = (L != null) ? L.size : 0;
        final TreeNode R = node.right;
        int rightSize = (R != null) ? R.size : 0;
    ...
    public static int height(final TreeNode node) {
        ...
        final TreeNode L = node.left;
        int lh = (L != null) ? height(L) : 0;
        final TreeNode R = node.right;
        int rh = (R != null) ? height(R) : 0;
        return Math.max(lh, rh) + 1;
    }

    public static TreeNode getLeftmostNode(final TreeNode node) {
        TreeNode n = node;
        while (n.left != null) {
            n = n.left;
        }
    ...
    public static TreeNode getRightmostNode(final TreeNode node) {
        TreeNode n = node;
        while (n.right != null) {
            n = n.right;
        }
