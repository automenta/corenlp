...
public class Counters {
    ...
    public static <E> double logSum(Counter<E> c) {
        return ArrayMath.logSum(c.valueArray());
    }
    ...
    public static <E> void logNormalizeInPlace(DefaultCounter<E> c) {
        final double logsum = logSum(c);
        c.map.forEachKey(key -> {
            c.incrementCount(key, -logsum);
        });
    }
    ...
    public static <E> double max(Counter<E> c, double valueIfEmpty) {
        if (c.size() == 0) {
            ...
            return c.maxValue(Double.NEGATIVE_INFINITY);
        }
    }
    ...
    public static <E> Counter<E> asCounter(Collection<E> c) {
        Counter<E> count = new DefaultCounter<>();
        for (E elem : c) {
    ...
    public static <E> double min(Counter<E> c) {
        if (c instanceof DefaultCounter) {
            return min((DefaultCounter)c);
        }
    ...
    public static <E> double min(DefaultCounter<E> c) {
        return c.map.minIfEmpty(Double.POSITIVE_INFINITY);
    }
    ...
    public static <E> double standardDeviation(DefaultCounter<E> c) {
        final double[] std = {0};
        double mean = c.totalCount() / c.size();
        c.map.forEachValue(v -> {
            double dv = v - mean;
            std[0] += dv * dv;
        });
        return Math.sqrt(std[0] / c.size());
    }
    ...
    public static <E> void addInPlace(Counter<E> target, Counter<E> arg) {
        if (arg instanceof DefaultCounter) {
            addInPlace(target, ((DefaultCounter)arg));
            return;
        }
    ...
    public static <E> void addInPlace(Counter<E> target, DefaultCounter<E> arg) {
        arg.map.forEachKeyValue((k, v) -> target.incrementCount(k, v));
    }
    ...
    public static <E> Counter<E> divideInPlace(DefaultCounter<E> target, double divisor) {
        return multiplyInPlace(target, 1.0/divisor);
    }
    public static <E> Counter<E> multiplyInPlace(DefaultCounter<E> target, double factor) {
        final ObjectDoubleHashMap<E> tm = target.map;
        tm.forEachKeyValue((k, v) -> target.setCount(k, v * factor) );
        return target;
    }
     ...
     */
    public static <E> DefaultCounter<E> asNormalizedCounter(Counter<E> c) {
    ...
    public static <E1, E2> Set<Pair<E1, E2>> retainAbove(
        ...
        for (Entry<E1, DefaultCounter<E2>> en : counter.entrySet()) {
            en.getValue().map.forEachKeyValue((k, v) -> {
                if (counter.getCount(en.getKey(), k) < countThreshold) {
                    removed.add(new Pair<>(en.getKey(), k));
                }
            });
        }
    ...
    public static <E> Counter<E> retainBelow(Counter<E> counter, double countMaxThreshold) {
        Counter<E> removed = new DefaultCounter<>();
        for (E key : counter.keySet()) {
    ...
    public static <T1, T2> Counter<T2> transform(Counter<T1> c, Function<T1, T2> f) {
        Counter<T2> c2 = new DefaultCounter<>();
        for (T1 key : c.keySet()) {
...
//    }
     ...
     */
    public static <E> DefaultCounter<E> toTiedRankCounter(Counter<E> c) {
        DefaultCounter<E> rankCounter = new DefaultCounter<>();
    ...
    public static <E> DefaultCounter<E> union(Counter<E> c1, Counter<E> c2) {
        DefaultCounter<E> result = new DefaultCounter();
     ...
     */
    public static <E> DefaultCounter<E>  intersection(Counter<E> c1, Counter<E> c2) {
        DefaultCounter<E> result = new DefaultCounter();
     ...
     */
    public static <E> DefaultCounter<E> product(Counter<E> c1, Counter<E> c2) {
        DefaultCounter<E> result = new DefaultCounter();
     ...
     */
    public static <E> DefaultCounter<E> add(Counter<E> c1, double value) {
        DefaultCounter<E> result = new DefaultCounter();
        for (E key : c1.keySet()) {
     ...
     */
    public static <E> DefaultCounter<E> absoluteDifference(Counter<E> c1, Counter<E> c2) {
        DefaultCounter<E> result = new DefaultCounter();
        for (E key : Sets.union(c1.keySet(), c2.keySet())) {
     ...
     */
    public static <E> DefaultCounter<E> L2Normalize(Counter<E> c) {
     ...
     * @return A new L2-normalized Counter based on c.
     */
    public static <E> DefaultCounter<E> saferL2Normalize(Counter<E> c) {
    ...
    public static <E> Counter<E> average(Counter<E> c1, Counter<E> c2) {
        Counter<E> average = new DefaultCounter<>();
...
//        Counter<Integer> countCounts = new DefaultCounter<>();
...
//    }
    ...
    public static <E> Counter<Double> getCountCounts(Counter<E> c) {
        Counter<Double> result = new DefaultCounter<>();
        for (double v : c.values()) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <E> DefaultCounter<E> scale(Counter<E> c, double s) {
        DefaultCounter<E> scaled = new DefaultCounter();
        for (E key : c.keySet()) {
     ...
     */
    public static <E> DefaultCounter<E> loadCounter(String filename, Class<E> c) throws RuntimeException {
        DefaultCounter<E> counter = new DefaultCounter<>();
    ...
    public static <T> DefaultCounter<T> deserializeCounter(String filename) throws Exception {
        // reconstitute
        ObjectInputStream in = new ObjectInputStream(new BufferedInputStream(new FileInputStream(filename)));
        DefaultCounter<T> c = ErasureUtils.uncheckedCast(in.readObject());
        in.close();
        return c;
    }
    ...
    public static <T> Counter<T> toCounter(double[] counts, Index<T> index) {
        ...
        Counter<T> c = new DefaultCounter<>();
    ...
    public static <E> Counter<E> toCounter(Map<Integer, ? extends Number> counts, Index<E> index) {
        ...
        Counter<E> counter = new DefaultCounter<>();
    ...
    public static <T1, T2> TwoDimensionalCounter<T1, T2> scale(TwoDimensionalCounter<T1, T2> c, double d) {
        ...
        for (T1 key : c.firstKeySet()) {
            DefaultCounter<T2> ctr = c.getCounter(key);
    ...
    public static <E> Counter<E> getCopy(Counter<E> originalCounter) {
        Counter<E> copyCounter = new DefaultCounter<>();
    ...
    public static <A, B> void divideInPlace(TwoDimensionalCounter<A, B> counter, double divisor) {
        for (Entry<A, DefaultCounter<B>> c : counter.entrySet()) {
            Counters.divideInPlace(c.getValue(), divisor);
        }
        counter.recomputeTotal();
    }

    public static <E> double pearsonsCorrelationCoefficient(DefaultCounter<E> x, DefaultCounter<E> y) {
        ...
        DefaultCounter<E> t1 = Counters.add(x, -meanX);
        DefaultCounter<E> t2 = Counters.add(y, -meanY);
    ...
    public static <E> double spearmanRankCorrelation(Counter<E> x, Counter<E> y) {
        DefaultCounter<E> xrank = Counters.toTiedRankCounter(x);
        DefaultCounter<E> yrank = Counters.toTiedRankCounter(y);
    ...
    public static <E> Counter<E> getFCounter(Counter<E> precision, Counter<E> recall, double beta) {
        Counter<E> fscores = new DefaultCounter<>();
    ...
    public static <E> Counter<E> getCounts(Counter<E> c, Collection<E> keys) {
        Counter<E> newcounter = new DefaultCounter<>();
        for (E k : keys)
    ...
    public static <E, E2> Counter<E> flatten(Map<E2, Counter<E>> hier) {
        Counter<E> flat = new DefaultCounter<>();
