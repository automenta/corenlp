...
public class TestSentence implements SequenceModel {
    ...
    private double[] getHistories(String[] tags, History h, IntObjectHashMap<Extractor> extractors, IntObjectHashMap<Extractor> extractorsRare) {
        if (maxentTagger.hasApproximateScoring())
            return getApproximateHistories(tags, h, extractors, extractorsRare);
        return getExactHistories(h, extractors, extractorsRare);
    }

    private double[] getExactHistories(History h, IntObjectHashMap<Extractor> extractors, IntObjectHashMap<Extractor> extractorsRare) {
        double[] scores = new double[maxentTagger.ySize];
        final int szCommon = maxentTagger.extractors.size();

        extractors.forEachKeyValue((kf, ex) -> {
            extracttt(h, scores, kf, ex);
        });

        if (extractorsRare != null) {
            extractorsRare.forEachKeyValue((kf, ex) -> {
                extracttt(h, scores, kf + szCommon, ex);
            });
        }
        return scores;
    }

    void extracttt(History h, double[] scores, int kf, Extractor ex) {
    ...
    private double[] getApproximateHistories(String[] tags, History h, IntObjectHashMap<Extractor> extractors, IntObjectHashMap<Extractor> extractorsRare) {
